...
// Numeric input question: user types the exact number; pct=true means user enters 0–100 and value is divided by 100 before storing
const qn = (id, text, sub, unit, placeholder, src, pct = false) => ({ id, text, sub, type: 'number', unit, placeholder, source: src, pct });
...
export const NICHES = [
...

  {
    ...
    questions: [
      qn('leads','How many inbound leads or inquiries do you receive per month?','Include calls, web forms, Zillow, Realtor.com, social media, and referrals','per month','e.g., 25','NAR 2023 Member Profile: Average agent receives 12–15 leads/month; top producers handle 50+'),
      qn('response','What percentage of new leads receive a personal response within 5 minutes?','Be honest — this single metric has the biggest impact on your close rate','%','e.g., 30','MIT/InsideSales.com: Leads contacted within 5 minutes are 9× more likely to convert than those reached after 30 minutes',true),
      qn('commission','What is your average gross commission per closed transaction?','Estimate based on your typical price range and market','$','e.g., 8000','NAR 2023: Median agent income $56,400/yr; commission varies widely by market and price point'),
      qn('admin','How many hours per week do you spend on administrative tasks?','Emails, CRM updates, paperwork, data entry — anything not directly client-facing','hrs/week','e.g., 15','ActivePipe 2022: Real estate agents spend an average of 35–40% of their working hours on administrative tasks'),
      qn('followup','What percentage of your follow-up for unconverted leads is automated?','0 = you personally reach out every time; 100 = fully automated sequences run without you','%','e.g., 30','Marketing Donut: 80% of sales require 5+ follow-up contacts; 44% of agents give up after just 1 attempt',true),
      qn('afterhours','What percentage of after-hours leads receive an automated or same-day response?','After 6pm, weekends, and holidays','%','e.g., 20','Zillow 2022: ~35% of all buyer inquiries are submitted outside normal business hours',true),
      qn('scheduling','How many hours per week do you spend coordinating showings and managing your calendar?','Scheduling, confirming, rescheduling — back-and-forth with buyers and sellers','hrs/week','e.g., 8','Lone Wolf Technologies 2023: Agents spend 8–12 hours/week on scheduling-related tasks'),
      qn('sphere','What percentage of your past clients do you proactively contact at least once per quarter?','Calls, texts, emails, market updates — any intentional outreach to your sphere','%','e.g., 25','NAR: 88% of buyers say they\'d use the same agent again — only 12% do. Lack of follow-up is the #1 reason.',true),
      qn('nurtures','How many active buyers or sellers are in your pipeline who haven\'t transacted yet?','People who need 3–12 more months before they\'re ready to act','people','e.g., 20','Industry avg: Most agents have 15–25 people in their pipeline who could close within 12 months with consistent nurturing'),
      qn('txnhours','How many hours does transaction coordination take per deal?','Paperwork, deadline tracking, lender comms, status updates to clients','hrs/deal','e.g., 15','Real Trends 2022: Average real estate transaction requires 15–20 hours of coordination work'),
      qn('closings','How many transactions do you close per month on average?','Divide quarterly total by 3 if easier','per month','e.g., 2','NAR 2023: Median agent closes 12 transactions/year (1/month); top 10% close 3+ per month'),
      qn('referrals','How many referrals do you receive from past clients per month?','People who called specifically because someone recommended you','per month','e.g., 2','NAR: 41% of buyers found their agent through a referral; systematic referral programs produce 3–5× more referral business'),
    ],
    calculate: (a) => {
      ...
      return {
        ...
        painPoints:[
          pp('Slow Lead Response',`$${fmt(missed*commission*0.05)}/mo lost`,`+$${fmt(missed*0.85*commission*0.12)}/mo recovered`,`${Math.round(missed)} leads/month aren't getting a 5-minute response. MIT research shows this single fix improves conversion 9×.`,'Virtual Front Desk responds to every lead within 30 seconds, 24/7, qualifies them, and books showings automatically'),
...

  {
    ...
    questions: [
      qn('calls','How many inbound calls or service requests do you receive per month?','Include phone calls, web form submissions, and online bookings','per month','e.g., 80','IBIS World 2023: Average HVAC company handles 60–120 service calls/month; larger companies 300+'),
      ...
      qn('scheduling','How many hours per week does your team spend on scheduling, dispatch, and rescheduling?','Back-and-forth with techs and customers to coordinate jobs','hrs/week','e.g., 10','ServiceTitan: Dispatching and scheduling accounts for 25–35% of office staff time in field service businesses'),
      qn('seasonal','What percentage of your peak-season call overflow is handled automatically?','Your ability to handle 2–3× normal call volume without dropping calls','%','e.g., 20','ACCA 2022: HVAC companies lose avg 18–35% of peak-season call revenue due to call overflow and slow response times',true),
      qn('reviews','How many online reviews do you receive per month on average?','Google, Yelp, Facebook, Angi','per month','e.g., 5','BrightLocal 2023: 98% of consumers read online reviews for local businesses; businesses with 50+ reviews earn 4.6× more revenue'),
    ],
    calculate: (a) => {
      ...
      return {
        ...
        weeklyHoursWasted:Math.round(wasted), weeklyHoursSaved:Math.round(saved), annualHoursSaved:Math.round(saved*52),
        painPoints:[
          pp('Missed & Unanswered Calls',`$${fmt(missedCalls*ticket*0.55)}/mo lost`,`+$${fmt(missedCalls*0.88*ticket*0.55)}/mo recovered`,`${Math.round(missedCalls)} calls/month go to voicemail. 80% of those callers immediately call your competitor.`,'Virtual Front Desk answers every call 24/7, books the job, and sends the tech details — no voicemail, no lost leads'),
...

  {
    ...
    questions: [
      qn('inquiries','How many new client inquiries or consultation requests do you receive per month?','Calls, web forms, referrals, and walk-ins','per month','e.g., 20','Clio Legal Trends Report 2023: Average law firm receives 25–40 new client inquiries per month'),
      qn('response','What percentage of inquiries receive a response within the same business day?','Calls returned, emails replied to, forms acknowledged','%','e.g., 50','Clio 2023: 42% of legal consumers hired the first attorney who responded to them — response speed is the #1 conversion factor',true),
      qn('casevalue','What is your average case or retainer value?','Estimate across all matter types you handle','$','e.g., 5000','Clio Legal Trends 2023: Average legal matter value ranges from $1,200 (family) to $35,000+ (business litigation)'),
      qn('billable','How many hours per week does your staff spend on non-billable administrative tasks?','Intake, scheduling, document prep, client updates, billing admin','hrs/week','e.g., 15','Clio 2023: Attorneys bill only 2.5 hours of an 8-hour day on average — the rest is non-billable admin and overhead'),
      qn('intake','How many calendar days does it take from first contact to a signed retainer?','From inquiry to signed agreement','days','e.g., 5','LawPay 2022: Every extra day in the intake process reduces conversion rate by approximately 8%'),
      qn('followup','What percentage of your follow-up for consultation no-shows or unconverted inquiries is automated?','People who showed interest but didn\'t retain','%','e.g., 20','Legal marketing data: 60% of law firm leads that don\'t convert immediately would hire within 3 months with consistent follow-up',true),
      qn('noshow','What percentage of scheduled consultations result in no-shows or cancellations?','Without re-booking','%','e.g., 18','Clio 2022: Law firm no-show rates average 18–22%; automated reminders reduce no-shows by up to 80%',true),
      qn('admin','How many hours per week do attorneys spend on tasks that don\'t require a law degree?','Client status updates, scheduling, basic document generation, billing follow-up','hrs/week','e.g., 10','McKinsey 2023: 23% of a lawyer\'s work is automatable — primarily document review, contract generation, and client communications'),
      qn('billing','What percentage of invoices are paid within 30 days without follow-up?','No calls or reminders needed','%','e.g., 50','LawPay 2023: Firms using automated billing reminders collect 35% faster and reduce outstanding receivables by 40%',true),
      qn('referrals','How many referrals do you receive from past clients per month on average?','Clients who called specifically because someone sent them','per month','e.g., 2','Clio 2023: 62% of legal consumers choose their attorney based on referrals — firms with post-case follow-up systems get 3× more referrals'),
      qn('reviews','How many new online reviews does your firm receive per month?','Google, Avvo, Martindale, Facebook','per month','e.g., 2','BrightLocal 2023: 87% of consumers read reviews for local services including legal; firms with 50+ reviews convert 4× more website visitors'),
    ],
    calculate: (a) => {
...

  {
    ...
    questions: [
      qn('patients','How many patient appointments do you schedule per month?','Across all providers in your practice','per month','e.g., 250','MGMA 2023: Average primary care physician sees 18–22 patients/day; specialists 12–18/day'),
      qn('noshow','What is your current patient no-show or same-day cancellation rate?','Appointments that happen without advance rescheduling','%','e.g., 18','MGMA 2023: Average medical practice no-show rate is 18–23%; automated reminders reduce this by 55–80%',true),
      qn('visitvalue','What is your average revenue per patient visit?','Include all billing codes, co-pays, and ancillary services','$','e.g., 200','CMS 2023: Average primary care visit reimbursement $150–$300; specialist visits $300–$800+ depending on specialty'),
      qn('newpatients','How many new patient inquiries per month do not convert to a first appointment?','People who called or filled out a form but never booked','per month','e.g., 15','Healthcare marketing data: 45% of new patient inquiries don\'t convert due to slow response or difficult booking process'),
      qn('admin','How many staff hours per week are spent on phone-based scheduling and administrative tasks?','Appointment booking, rescheduling, insurance verification, reminder calls','hrs/week','e.g., 30','MGMA 2022: Front desk staff spend 40–55% of their time on scheduling-related phone calls'),
      qn('recall','What percentage of patients due for follow-up or preventive care actually return on schedule?','Annual physicals, chronic disease management, post-procedure follow-ups','%','e.g., 50','CDC/AHRQ: Patient recall programs that use automated outreach achieve 40–60% higher adherence rates than manual systems',true),
      qn('afterhours','What percentage of after-hours patient inquiries and appointment requests receive an automated or timely response?','New patient inquiries or scheduling requests after office hours','%','e.g., 30','Accenture 2021: 68% of patients prefer self-scheduling; 30% of online scheduling happens outside business hours',true),
      qn('insurance','How many hours per week does your staff spend on insurance verification and pre-authorization?','Including denials, re-submissions, and follow-ups','hrs/week','e.g., 15','MGMA 2023: Prior authorization and insurance admin costs practices avg $14.30 per physician per hour spent'),
      qn('reviews','How many new online reviews does your practice receive per month?','Google, Healthgrades, Yelp, ZocDoc','per month','e.g., 5','Software Advice 2023: 94% of patients use online reviews to evaluate physicians; practices with 50+ reviews see 30% more new patient conversions'),
      qn('satisfaction','What percentage of patients receive an automated satisfaction survey after their visit?','Post-visit surveys, NPS, or similar','%','e.g., 20','HCAHPS/Press Ganey: Practices with systematic satisfaction collection and response see 15–25% higher patient retention',true),
    ],
    calculate: (a) => {
...

  {
    ...
    questions: [
      qn('patients','How many patient appointments do you schedule per month?','New patients plus recall/hygiene plus treatment','per month','e.g., 200','ADA 2023: Average general dental practice sees 10–18 patients/day; specialty practices vary widely'),
      qn('noshow','What is your patient no-show and same-day cancellation rate?','Appointments that block chair time but produce no revenue','%','e.g., 15','ADA Health Policy Institute 2023: Dental no-show rates average 14–19%; automated reminders reduce this by up to 75%',true),
      qn('visitvalue','What is your average revenue per patient visit?','Include all procedures; estimate across all appointment types','$','e.g., 400','ADA 2023: Average dental procedure revenue $300–$500/visit; treatment plan cases $800–$5,000+'),
      qn('recall','What percentage of your active patients return for their 6-month hygiene recall on schedule?','Without your team having to chase them repeatedly','%','e.g., 50','ADA: Only 47% of dental patients return for recall appointments on schedule without automated outreach',true),
      qn('treatmentplans','What percentage of presented treatment plans are accepted and scheduled within 30 days?','Cases where patient said yes and booked','%','e.g., 35','Dental consulting data: Average treatment plan acceptance rate is 30–40%; practices with follow-up automation see 55–65% acceptance',true),
      qn('newpatients','How many new patient inquiries do not convert to a first appointment each month?','Calls or forms that never resulted in a booking','per month','e.g., 10','Dental marketing data: 40% of new patient inquiries don\'t book due to slow response or difficult scheduling process'),
      qn('admin','How many staff hours per week are spent on phone-based scheduling, reminders, and admin?','Including recall calls, appointment reminders, insurance verification','hrs/week','e.g., 25','ADA 2022: Dental front desk staff spend avg 35–45% of their time on scheduling and reminder phone calls'),
      qn('insurance','How many hours per week does your team spend on insurance billing and follow-up?','Claims, pre-authorizations, denials, and patient billing questions','hrs/week','e.g., 12','ADAA 2023: Insurance billing admin consumes 15–25% of dental practice overhead costs'),
      qn('reviews','How many new online reviews does your practice receive per month?','Google, Yelp, Healthgrades','per month','e.g., 3','Software Advice 2023: 72% of patients check online reviews before choosing a dentist; practices with 50+ reviews see significantly more new patients'),
      qn('afterhours','What percentage of after-hours patient calls and new patient inquiries receive an automated or timely response?','Evenings, weekends','%','e.g., 25','DentistryIQ: 35% of dental appointment requests happen outside business hours — most go to practices with 24/7 online booking',true),
    ],
    calculate: (a) => {
    ...
    automationPlan:[
      ...
      wf('New Patient Experience Automation','high','New patient inquiry via any channel',['Inquiry acknowledged within 2 minutes — 24/7','New patient forms and insurance info sent immediately','Insurance verified before first visit automatically','Welcome package sent: what to expect, forms, parking, team bios','Post-first-visit check-in and review request at 24 hours'],['Virtual Front Desk','Dental Software Integration','Insurance Verification','Email Automation'],'Convert 60% more inquiries and create a first impression that earns 5-star reviews before they even leave the chair'),
...

  {
    ...
    calculate: (a) => {
      ...
      return {
        ...
        weeklyHoursWasted:Math.round(wasted), weeklyHoursSaved:Math.round(saved), annualHoursSaved:Math.round(saved*52),
        painPoints:[
          pp('Missed Calls & Slow Response',`$${fmt(leads*missed*jobVal*0.35)}/mo lost`,`+$${fmt(leads*missed*0.85*jobVal*0.45)}/mo recovered`,`${Math.round(leads*missed)} calls/month go unanswered. In roofing, the first contractor to pick up typically wins the job.`,'Virtual Front Desk answers every call, qualifies the lead (storm damage vs. standard repair), and schedules the estimate immediately'),
    ...
    automationPlan:[
      wf('Instant Lead Capture & Estimate Scheduling','critical','New inbound call or web form',['Virtual Front Desk answers 24/7, qualifies lead type (storm, repair, re-roof)','Estimate appointment scheduled on the spot','Estimator notified with lead details and job address','Pre-appointment follow-up sent to homeowner with what to expect','Post-estimate: proposal sent automatically with DocuSign link'],['Virtual Front Desk','Scheduling Automation','CRM Integration','E-Signature Platform'],'Capture 85% of leads currently going unanswered and cut estimate-to-signed-contract time by 60%'),
...

  {
    ...
    automationPlan:[
      wf('Instant Quote Response & Nurture','critical','New prospect inquiry',['Inquiry acknowledged within 60 seconds','Quote request form sent instantly','Quote delivered automatically where possible (comparative rater integration)','If not immediate: follow-up at 2hr, 24hr, 72hr, 7 days, 30 days, 60 days, 90 days','Objection-handling content sent based on response patterns'],['Virtual Front Desk','Comparative Rater Integration','Email + SMS Sequences','CRM'],'Convert 40% more prospects from quote to policy — the highest-value automation for any insurance agency'),
...

  {
    ...
    questions: [
      qn('prospects','How many new prospect inquiries do you receive per month?','Referrals, website, seminars, social media','per month','e.g., 10','Cerulli Associates 2023: Average RIA gets 70% of new clients from referrals; 10–15 new prospects/month typical'),
      qn('response','What percentage of prospects receive a same-day response?','','%','e.g., 50','Financial advisor data: Prospects who receive same-day response convert at 3× the rate of those waited on',true),
      qn('aum','What is your average AUM per new client?','','$','e.g., 400000','Cerulli 2023: Median new client AUM for RIAs $250K–$500K; varies widely by firm positioning'),
      qn('fee','What is your average annual advisory fee as a percentage of AUM?','Enter as a percentage — e.g., enter 1 for a 1% AUM fee','%','e.g., 1.0','Industry standard: Advisory fees typically 0.75–1.25% of AUM for full-service wealth management',true),
      qn('admin','How many hours per week do you spend on non-revenue-generating admin?','Client communications, scheduling, compliance documentation, reporting prep','hrs/week','e.g., 15','InvestmentNews 2022: Advisors spend avg 40% of their time on admin tasks vs. revenue-generating client activities'),
      qn('clientmeetings','How many client review meetings do you hold per month?','','per month','e.g., 20','CFP Board: Best practice is semi-annual reviews for all clients; most advisors fall short due to scheduling burden'),
      qn('referrals','How many referrals do you receive from existing clients per month?','','per month','e.g., 2','Advisor data: Only 29% of clients who would refer their advisor ever actually do — the gap is the advisor never asked'),
      qn('followup','What percentage of your prospect follow-up is automated?','For prospects who didn\'t immediately engage','%','e.g., 20','Financial planning data: 45% of prospects who initially decline engage within 6 months with consistent, value-add follow-up',true),
      qn('reporting','How many hours per week do you spend on client reporting, portfolio review prep, and statement generation?','','hrs/week','e.g., 8','Orion/Redtail data: Reporting and prep consumes 15–25% of advisor operational hours'),
      qn('compliance','How many hours per week do you spend on compliance documentation and audit trail maintenance?','','hrs/week','e.g., 5','SEC compliance data: RIAs spend avg 6–12% of operational time on compliance documentation'),
    ],
    calculate: (a) => {
  ...
  ...([
    ...
    calculate: (a) => {
      ...
      return {
        ...
        weeklyHoursWasted:Math.round(wasted), weeklyHoursSaved:Math.round(saved), annualHoursSaved:Math.round(saved*52),
        painPoints:[
          pp('Slow or Missed Responses',`$${fmt(leads*(1-response)*avgVal*0.06)}/mo lost`,`+$${fmt(leads*(1-response)*0.8*avgVal*0.12)}/mo recovered`,`${Math.round(leads*(1-response))} inquiries/month not receiving same-day responses. Same-day response increases conversion by 2–4×.`,'Virtual Front Desk or automated response system acknowledges every inquiry within minutes and routes to the right person instantly'),
    ...
    automationPlan:[
      wf('Instant Response & Lead Nurture','critical','New inquiry from any channel',['Inquiry acknowledged within 60–120 seconds 24/7','Qualification questions sent automatically','Appointment or next step booked directly','Hot leads routed to team immediately','Cold leads enter automated nurture sequence'],['Virtual Front Desk','CRM Integration','SMS + Email Automation','Scheduling Tool'],'Capture every lead at the moment of highest intent — when they first reach out'),
  ...
  ...([
    ...
    questions: [
      ...
      qn('afterhours', 'What percentage of after-hours inquiries receive an automated or timely response?', '', '%', 'e.g., 20', '30–40% of consumer inquiries arrive outside business hours', true),
      qn('scheduling', 'How many hours per week do you spend on scheduling and booking?', '', 'hrs/week', 'e.g., 8', 'Scheduling and booking admin consumes 20–30% of service business operational time'),
      qn('retention', 'What percentage of customers return or book again within 12 months?', '', '%', 'e.g., 35', 'Bain & Company: 5% improvement in retention increases profits by 25–95%', true),
    ],
    calculate: (a) => {
    ...
    automationPlan:[
      wf('Instant Response & Booking System','critical','New inquiry via any channel', ['Inquiry acknowledged within 2 minutes 24/7','Availability and pricing info sent automatically','Booking link or calendar offered immediately','Confirmation and pre-appointment info sent','Reminder sequence started automatically'], ['Virtual Front Desk','Online Booking Integration','SMS + Email Automation','CRM'], 'Capture every inquiry at peak intent and convert it into a confirmed booking'),
...

  {
    ...
    questions: [
      qn('leads','How many inbound leads or inquiries do you receive per month?','Include calls, web form submissions, social media messages, referrals, and walk-ins','per month','e.g., 40','Harvard Business Review: Average small business receives 30–50 inbound inquiries per month across all channels'),
      qn('response','What percentage of new leads receive a response within the same business day?','Be honest — delayed responses are the #1 reason leads go cold','%','e.g., 50','MIT/InsideSales.com: Leads contacted within 5 minutes convert 9× better than those reached after 30 minutes; same-day responses outperform next-day by 4×',true),
      qn('avgval','What is your average transaction or job value?','Estimate the typical revenue you receive per customer transaction or project','$','e.g., 500','SCORE 2023: Average small business transaction value varies widely by industry; general service businesses average $500–$2,500'),
      qn('admin','How many hours per week do you spend on administrative tasks?','Emails, scheduling, invoicing, data entry, follow-ups — anything not directly serving clients','hrs/week','e.g., 15','McKinsey 2022: Small business owners spend an average of 40% of their time on administrative tasks that could be automated'),
      qn('followup','What percentage of your lead follow-up process is automated?','0 = you personally reach out every time; 100 = software handles it on a schedule','%','e.g., 20','Marketing Donut: 80% of sales require 5+ follow-up contacts; 44% of sales reps give up after just one attempt',true),
      qn('afterhours','What percentage of after-hours leads and inquiries receive an automated or timely response?','Evenings, weekends, and holidays','%','e.g., 20','Drift 2022: 33% of all business inquiries are submitted outside normal business hours; 82% of consumers expect a response within 10 minutes',true),
      qn('scheduling','How many hours per week do you spend coordinating scheduling and calendar management?','Booking appointments, confirming, rescheduling — back-and-forth with clients and vendors','hrs/week','e.g., 5','Calendly 2023: Businesses spend an average of 4.8 hours per week on scheduling-related communications'),
      qn('retention','What percentage of your customers return for another purchase or project within 12 months?','Repeat buyers, renewal clients, or returning customers','%','e.g., 35','Bain & Company: A 5% increase in customer retention increases profits by 25–95%; acquiring new customers costs 5× more than retaining existing ones',true),
      qn('referrals','How many referrals do you receive from past clients or customers per month?','People who specifically contacted you because someone recommended your business','per month','e.g., 2','Nielsen: 92% of consumers trust referrals from people they know; systematic referral programs produce 3–5× more referral business'),
      qn('reviews','What percentage of your customers receive an automated review request after completing a transaction?','0 = you never ask; 100 = every customer gets an automated review request','%','e.g., 10','BrightLocal 2023: 87% of consumers read online reviews for local businesses; businesses with 4.5+ stars receive 28% more clicks',true),
    ],
    calculate: (a) => {
      ...
      return {
        ...
        weeklyHoursWasted:Math.round(wasted), weeklyHoursSaved:Math.round(saved), annualHoursSaved:Math.round(saved*52),
        painPoints:[
          pp('Slow Lead Response',`$${fmt(leads*(1-response)*avgVal*0.05)}/mo lost`,`+$${fmt(leads*(1-response)*0.85*avgVal*0.11)}/mo recovered`,`${Math.round(leads*(1-response))} leads/month aren't getting a same-day response. The first business to respond wins the sale in most industries.`,'Virtual Front Desk responds to every inquiry within 30 seconds — 24/7 — qualifying leads and routing hot prospects automatically'),
